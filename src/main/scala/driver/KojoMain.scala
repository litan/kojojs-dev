...
"""
  ...
  def picDomImage(): Unit = {
    ...
    cleari()
    val urlBase = "https://kojofiles.netlify.app"
    val img = image(s"$urlBase/collidium/ball1.png")
  ...
  def picSpriteSheet(): Unit = {
    ...
    def pictureFromImagesE(images: ArrayBuffer[Image], env: Picture) = {
      val pics = ArrayBuffer.empty[Picture]
    ...
    def pictureFromImages(images: ArrayBuffer[Image]) = {
      val pics = ArrayBuffer.empty[Picture]
      repeatFor(images) { img =>
        pics.append(Picture.image(img))
      }
      picBatch(pics)
    }

    val picImages = sheetImages(s"$urlBase/robot_walk_sheet.png", 8, 96, 128)
    val pic = pictureFromImages(picImages)

    draw(pic)

    var vel = Vector2D(2, 1)
    animate {
      pic.translate(vel)
      pic.showNext(200)
      if (pic.collidesWith(stageBorder)) {
  ...
  def ellipse(): Unit = {
    ...
    clear()
    val pic = Picture.ellipse(200, 100)
    draw(pic)
  }

  def ellipseInRect(): Unit = {
    import kojo.{SwedishTurtle, Turtle, KojoWorldImpl, Vector2D, Picture}
    import kojo.doodle.Color._
    import kojo.Speed._
    import kojo.RepeatCommands._
